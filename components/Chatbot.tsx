...
import { X, Send, Loader2, Maximize2, Minimize2, History, MessageSquare, Plus, Trash2, Download } from 'lucide-react';
  ...
  const exportToPDF = async () => {
    ...
    messages.forEach((message, index) => {
      ...
      if (message.role === 'user') {
        pdf.setTextColor(3, 109, 173); // Blue for user
        pdf.text('You:', margin, yPosition);
      } else {
        pdf.setTextColor(34, 139, 34); // Green for assistant
